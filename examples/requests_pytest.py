...
from xjsonrpc.client.backend import requests as requests_client


def test_using_fixture(xjsonrpc_requests_mocker):
    client = requests_client.Client('http://localhost/api/v1')

    xjsonrpc_requests_mocker.add('http://localhost/api/v1', 'sum', result=2)
    result = client.proxy.sum(1, 1)
    assert result == 2

    xjsonrpc_requests_mocker.replace(
    ...
    localhost_calls = xjsonrpc_requests_mocker.calls['http://localhost/api/v1']
...
def test_using_resource_manager():
    ...
    with PjRpcRequestsMocker() as mocker:
        ...
        with pytest.raises(xjsonrpc.exc.MethodNotFoundError):
