...
import xjsonrpc
from xjsonrpc.client import AbstractClient, AbstractAsyncClient
...
class Client(AbstractClient):
    ...
    :param kwargs: parameters to be passed to :py:class:`xjsonrpc.client.AbstractClient`
    """
    ...
    def _request(self, request_text: str, is_notification: bool = False, **kwargs: Any) -> Optional[str]:
        ...
        kwargs = {
            'headers': {'Content-Type': xjsonrpc.common.DEFAULT_CONTENT_TYPE},
            **kwargs,
        }
        ...
        if response_text and content_type.split(';')[0] not in xjsonrpc.common.RESPONSE_CONTENT_TYPES:
            raise xjsonrpc.exc.DeserializationError(f"unexpected response content type: {content_type}")
