...
or by sending handmade :py:class:`xjsonrpc.common.Request` class object. Notification requests can be made using
:py:meth:`xjsonrpc.client.AbstractClient.notify` method or by sending a :py:class:`xjsonrpc.common.Request` object without id.
...
Batch requests also supported. You can build :py:class:`xjsonrpc.common.BatchRequest` request by your hand and then send
it to the server. The result is a :py:class:`xjsonrpc.common.BatchResponse` instance you can iterate over to get all
...
is not a :py:class:`xjsonrpc.common.BatchResponse` object anymore but a tuple of "plain" method invocation results):
...
which can be found in :py:mod:`xjsonrpc.common.exceptions` module so that error handling is very simple and "pythonic-way":
